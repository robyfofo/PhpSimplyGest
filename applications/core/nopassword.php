 ...
 * core/nopassword.php v.1.0.0. 04/07/2018
*/
		...
		if(Core::$resultOp->error == 0) {
			if (Sql::getFoundRows() > 0) {
				...
				if (Core::$resultOp->error == 0) {	
					...
					Mails::sendMail($address,$subject,$content,$text_content,array('fromEmail'=>$globalSettings['default email'],'fromLabel'=>$globalSettings['dafault email label']));							
					if (Core::$resultOp->error == 0) {					
