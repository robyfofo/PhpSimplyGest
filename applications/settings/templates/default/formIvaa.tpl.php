	...
	<div class="col-lg-12">	
		<form id="applicationForm" class="form-horizontal" role="form" action="{{ URLSITE }}{{ CoreRequest.action }}/{{ App.methodForm }}"  enctype="multipart/form-data" method="post">
			...
			<ul class="nav nav-tabs">
				<li class="{% if App.tabActive == 1 %}active{% endif %}"><a href="#datibase-tab" data-toggle="tab">{{ Lang['dati base']|capitalize }}</a></li>
				<li class=""><a href="#content-tab" data-toggle="tab">{{ Lang['contenuto']|capitalize }}</a></li>
				{% if App.id > 0 %}
				<li class="{% if App.tabActive == 2 %}active{% endif %}"><a href="#articles-tab" data-toggle="tab">{{ Lang['articoli associati']|capitalize }}</a></li>
				{% endif %}
				...
				<div class="tab-pane{% if App.tabActive == 1 %} active{% endif %}" id="datibase-tab">			
					<fieldset>
						...
						<div class="form-group">
							...
							<div class="col-md-4">
								<label for="nameID" class="col-md-2 control-label">{{ Lang['oppure'] }}</label>
								<select name="id_customer" id="id_customerID" class="form-control">
									<option value="0">{{ Lang['non è cliente']|capitalize }}</option>
								...
								{% if App.item_articles is iterable and App.item_articles|length > 0 %}
									{% for key,value in App.item_articles %}
										<tr>
											...
											<td class="actions">
												<a class="btn btn-default btn-circle modifyArtItem" data-id="{{ value.id }}" href="javascript:void(0);" title="{{ Lang['modifica %ITEM%']|replace({'%ITEM%': Lang['articolo']})|capitalize }}"><i class="fa fa-edit"> </i></a>
												<a class="btn btn-default btn-circle confirm" href="{{ URLSITE }}{{ CoreRequest.action }}/deleteArtItem/{{ App.id }}/{{ value.id }}" title="{{ Lang['cancella %ITEM%']|replace({'%ITEM%': Lang['articolo']})|capitalize }}"><i class="fa fa-cut"> </i></a>
											</td>							
										</tr>	
									{% endfor %}
								{% endif %}
						...
						<div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12">
							<div class="panel panel-info" id="articlePanelID">
					  			<div class="panel-heading" id="articlePanelHeadingID">
					    			<h3 class="panel-title" id="articlePanelTitleID">{{ Lang['inserisci %ITEM%']|replace({'%ITEM%': Lang['articolo']})|capitalize }}</h3>
					  			</div>
					  			<div class="panel-body form-inline">
										...
										<div class="row modalaction">					
											<div class="col-md-12 text-center">							
												...
												<button type="submit" name="submitArtForm" id="submitArtFormID" value="submitArt" class="btn btn-primary btn-sm">{{ Lang['aggiungi']|capitalize }}</button>
												<button type="button" name="resetArtForm" id="resetArtFormID" value="reset" class="pull-right btn btn-info btn-sm">{{ Lang['resetta']|capitalize }}</button>
											</div>
										</div>
