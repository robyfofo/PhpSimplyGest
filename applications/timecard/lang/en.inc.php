<?php
/**
 * Framework App PHP-MySQL
 * PHP Version 7
 * @author Roberto Mantovani (<me@robertomantovani.vr.it>
 * @copyright 2009 Roberto Mantovani
 * @license http://www.gnu.org/copyleft/lesser.html GNU Lesser General Public License
 * Timecard language EN v.1.0.0. 04/03/2019
*/

$_lang['pagetitle'] = "Custom timecard";
$_lang['pagesubtitle'] = "page of user custom timecard";
$_lang['voce']  = "timecard";
$_lang['voci']  = "timecard";
$_lang['la voce']  = "the timecard";
$_lang['le voci']  = "the timecard";
$_lang['inserisci una data globale'] = "enter a global data";
$_lang['vai a questa data'] = "select this data";
$_lang['ore totali'] = "ore totali";
$_lang['inserisci ora inizio'] = "enter start hour";
$_lang['inserisci ora fine'] = "enter finish hour";
$_lang['inserisci una timecard predefinita'] = "insert a custom timecard";
$_lang['gestisci le timecard predefinite'] = "manage custom timecard";
$_lang['usa questo inizio'] = "use this start";
$_lang['lista delle voci custom'] = "lista delle timecard predefinite dall'utente";
$_lang['lista delle voci'] = "lista delle timecard";
$_lang['archivio timecard'] = "archivio timecard";
$_lang['ora inizio'] = "start hour";
$_lang['ora fine'] = "end hour";
$_lang['ore lavoro'] = "working hours";
$_lang['La ora inizio deve essere prima della ora fine!'] =  "The start time must be before the end time!";
$_lang['La ora fine inserita non è valida!'] = "The end time inserted is invalid!";
$_lang['La ora inizio inserita non è valida!'] = "The start time inserted is invalid!";
$_lang['Partenza - Ore:Minuti'] = "Start";
$_lang['Fine - Ore:Minuti'] = "End";
$_lang['Timecard non trovata!'] = "Timecard non trovata!";
$_lang['Devi selezionare una timecard!'] = "Devi selezionare una timecard!";
$_lang['Devi selezionare un progetto!'] = "Devi selezionare un progetto!";
$_lang['La ora inizio deve essere prima della ora fine!'] = "La ora inizio deve essere prima della ora fine!";
?>