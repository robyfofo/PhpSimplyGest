<!-- app/users/listItem.tpl.php v.1.0.0. 05/07/2018 -->
<div class="row">
	<div class="col-md-3 new">
 		<a href="{{ URLSITE }}{{ CoreRequest.action }}/newItem" title="{{ Lang['inserisci un nuovo %ITEM%']|replace({'%ITEM%': Lang['utente']})|capitalize }}" class="btn btn-primary">{{ Lang['nuovo %ITEM%']|replace({'%ITEM%': Lang['utente']})|capitalize }}</a>
	</div>
						...
						<thead>
							<tr>
								{% if (App.userLoggedData.is_root is defined) and (App.userLoggedData.is_root is same as(1)) %}	
									<th class="id">ID</th>							
								{% endif %}	
								<th>{{ Lang['nome utente']|capitalize }}</th>
								<th>{{ Lang['livello']|capitalize }}</th>
								<th></th>
						...
						<tbody>				
									...
									<tr>
										...
										<td>										
											{% if (value.avatar is defined) and (value.avatar != '') %}
												<img src="{{ URLSITE }}users/renderAvatarDBItem/{{ value.id }}" alt="" style="max-height:70px;">
			            				{% endif %}
										</td>										
										<td>{{ value.name }}</td>
										<td>{{ value.surname }}</td>
							...
							{% else %}
								<tr>
									...
									<td colspan="8">{{ Lang['nessuna voce trovata!']|capitalize }}</td>
								</tr>
							{% endif %}
						</tbody>
					</table>
