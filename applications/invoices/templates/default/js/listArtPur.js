/* invoices/listArtPur.js v.1.0.0. 13/04/2018 */
...
$('.modifyArtPur').on('click',function(event) {
		var $id = $(this).data("id");	
		//alert(id);
		/* preleva i dati */
		if ($id > 0) {
			$.ajax({
				url: siteUrl+CoreRequestAction+'/getArticleAjaxArtPur',
				async: "true",
				cache: "false",
				type: "POST",
				data: {'id':$id},
			...
			.done(function(data) {	
				...
				$('#id_articleID').val(data.id);
				
				var $url = siteUrl+CoreRequestAction+'/updateArtPur';
