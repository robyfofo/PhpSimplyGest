								...
								{% if App.item_articles is iterable and App.item_articles|length > 0 %}
									{% for key,value in App.item_articles %}
										<tr>
											...
											<td class="actions">
												<a class="btn btn-default btn-circle modifyArtInvPur" data-id="{{ value.id }}" href="javascript:void(0);" title="{{ Lang['modifica %ITEM%']|replace({'%ITEM%': Lang['articolo']})|capitalize }}"><i class="fa fa-edit"> </i></a>
												<a class="btn btn-default btn-circle confirm" href="{{ URLSITE }}{{ CoreRequest.action }}/deleteArtInvPur/{{ App.id }}/{{ value.id }}" title="{{ Lang['cancella %ITEM%']|replace({'%ITEM%': Lang['articolo']})|capitalize }}"><i class="fa fa-cut"> </i></a>
											</td>							
										</tr>	
									{% endfor %}
								{% endif %}
