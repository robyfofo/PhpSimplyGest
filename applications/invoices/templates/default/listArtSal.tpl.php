...
<div class="row">
	...
	<div class="col-md-2">
		<a href="{{ URLSITE }}{{ CoreRequest.action }}/listInvSal" title="{{ Lang['vai alla lista delle voci']|capitalize }}" class="btn btn-primary">{{ Lang['lista voci']|capitalize }}</a>
	</div>
</div>
		...
		<div class="table-responsive">
			<table class="table table-striped table-bordered table-hover listData">
					...
					{% if App.items is iterable and App.items|length > 0 %}
						{% for key,value in App.items %}
							<tr>
								...
								<td class="actions">
									<a class="btn btn-default btn-circle" href="{{ URLSITE }}{{ CoreRequest.action }}/{{ value.active == 1 ? 'disactive' : 'active' }}ArtSal/{{ value.id  }}" title="{{ value.active == 1 ? App.lang['disattiva %ITEM%']|replace({'%ITEM%': App.lang['articolo']})|capitalize : App.lang['attiva %ITEM%']|replace({'%ITEM%': App.lang['articolo']})|capitalize }}"><i class="fa fa-{{ value.active == 1 ? 'unlock' : 'lock' }}"> </i></a>			 
									<a class="btn btn-default btn-circle modifyArtSal" data-id="{{ value.id }}" href="javascript:void(0);" title="{{ Lang['modifica %ITEM%']|replace({'%ITEM%': App.lang['articolo']})|capitalize }}"><i class="fa fa-edit"> </i></a>
									<a class="btn btn-default btn-circle confirm" href="{{ URLSITE }}{{ CoreRequest.action }}/deleteArtSal/{{ value.id }}" title="{{ Lang['cancella %ITEM%']|replace({'%ITEM%': App.lang['articolo']})|capitalize }}"><i class="fa fa-cut"> </i></a>
								</td>							
							</tr>	
						{% endfor %}
					{% else %}
				...
				<tfoot>
					...
					<td class="text-right">{{ App.tot_price_total_label }}</td>
					<td></td>								
	...
	<div class="col-lg-12">
		...
		<div class="panel panel-info" id="articlePanelID">
  			...
  			<div class="panel-body">
				...
				<form class="form-inline" id="articleFormID" role="form" action="{{ URLSITE }}{{ CoreRequest.action }}/insertArtSal" enctype="multipart/form-data" method="POST">
					<div class="row">	
						<div class="col-xs-12">
