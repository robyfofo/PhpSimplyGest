 ...
 * invoices/article-purchase.php v.1.0.0. 11/04/2018
*/
...
if (Core::$request->method == 'listArtPur' && $App->id > 0) $_MY_SESSION_VARS = $my_session->addSessionsModuleSingleVar($_MY_SESSION_VARS,$App->sessionName,'id_invoice',$App->id);
...
Sql::initQuery($App->params->tables['InvPur'],array('*'),array($App->id_invoice),'id = ?');
$App->invoice = Sql::getRecord();
if (Core::$resultOp->error == 0) {
	if (!isset($App->invoice->id) || (isset($App->invoice->id) && $App->invoice->id == 0)) {
		ToolsStrings::redirect(URL_SITE.Core::$request->action.'/messageInvPur/2/'.urlencode($_lang['Fattura non trovata!']));
		die();
		}
	} else {
		ToolsStrings::redirect(URL_SITE.Core::$request->action.'/messageInvPur/2/'.urlencode($_lang['Errore lettura dati fattura!']));
		die();
		}

/* preleva customer */
Sql::initQuery($App->params->tables['cust'],array('*'),array(),'');
Sql::setOptions(array('fieldTokeyObj'=>'id'));
$App->invoice_thirdparty = Sql::getRecords();
if (Core::$resultOp->error > 0) {
		ToolsStrings::redirect(URL_SITE.Core::$request->action.'/messageInvPur/2/'.urlencode($_lang['Errore lettura dati cliente!']));
		die();
		}						

switch(Core::$request->method) {
	case 'activeArtPur':
	case 'disactiveArtPur':
		Sql::manageFieldActive(substr(Core::$request->method,0,-6),$App->params->tables['ArtPur'],$App->id,array('labelA'=>$_lang['articolo attivato'],'labelD'=>$_lang['articolo disattivato']));
		$App->viewMethod = 'list';		
	break;

	case 'deleteArtPur':
		if ($App->id > 0) {
			Sql::initQuery($App->params->tables['ArtPur'],array('id'),array($App->id),'id = ?');
			//Sql::deleteRecord();
			if (Core::$resultOp->error == 0) {
	...
	case 'updateArtPur':
		...
		if (isset($_POST)) {	
			...
			Form::parsePostByFields($App->params->fields['ArtPur'],$_lang,array());	
			if (Core::$resultOp->error == 0) {
				Sql::updateRawlyPost($App->params->fields['ArtPur'],$App->params->tables['ArtPur'],'id',$_POST['id_article']);	
				if (Core::$resultOp->error == 0) {
...
switch((string)$App->viewMethod) {
	...
	case 'list':
		...
		$App->pageSubTitle = $_lang['lista degli articoli acquisti'];
		$App->templateApp = 'listArtPur.tpl.php';
		...
		$App->jscript[] = '<script src="'.URL_SITE.$App->pathApplications.Core::$request->action.'/templates/'.$App->templateUser.'/js/listArtPur.js" type="text/javascript"></script>';
	break;
	}
?>