<?php
/**
 * Framework App PHP-MySQL
 * PHP Version 7
 * @author Roberto Mantovani (<me@robertomantovani.vr.it>
 * @copyright 2009 Roberto Mantovani
 * @license http://www.gnu.org/copyleft/lesser.html GNU Lesser General Public License
 * language IT v.1.3.0. 14/09/2020
*/

$_lang['user'] = 'it';
$_lang['table_suffix'] = 'it';
$_lang['label'] = 'italiano';
$_lang['label_abb'] = 'ita';
$_lang['decimal separator'] = ",";
$_lang['data format'] = "d/m/Y";
$_lang['data format string'] = "%DAY% %STRINGMONTH% %YEAR%";
$_lang['data time format string'] = "%DAY% %STRINGMONTH% %YEAR% %HH%:%II%";
$_lang['data format string alt'] = "%STRINGDATADAY% %DAY% %STRINGMONTH% %YEAR%";
$_lang['datepicker data time format'] = "d/m/Y H:i";
$_lang['datepicker data format'] = "d/m/Y";
$_lang['datepicker time format'] = "H:i";
$_lang['charset'] = 'it-IT';
$_lang['charset date'] = 'it_IT';

// labels
$_lang['months'] = array('1' => 'gennaio','2' => 'febbraio','3' => 'marzo','4' => 'aprile','5' => 'maggio','6' => 'giugno','7' => 'luglio','8' => 'agosto','9' => 'settembre','10' => 'ottobre','11' => 'novenbre','12' => 'dicembre');
$_lang['days'] = array('0' => 'domenica','1' => 'lunedì','2' => 'martedì','3' => 'mercoledì','4' => 'giovedi','5' => 'venerdì','6' => 'sabato');
$_lang['anno'] = "anno";
$_lang['codice'] = "codice";
$_lang['aiuto'] = "aiuto";
$_lang['aiuto breve'] = "aiuto breve";
$_lang['hash'] = "hash";
$_lang['sezione'] = "sezione";
$_lang['sezioni'] = "sezioni";
$_lang['etichetta'] = "etichetta";
$_lang['etichette'] = "etichette";
$_lang['alias'] = "alias";
$_lang['ordinamento'] = "ordinamento";
$_lang['ordine'] = "ordine";
$_lang['ordini'] = "ordini";
$_lang['voci per pagina'] = "voci per pagina";
$_lang['precedente'] = "precedente";
$_lang['prossimo'] = "prossimo";
$_lang['prossima'] = "prossima";
$_lang['prossimi'] = "prossimi";
$_lang['prossime'] = "prossime";
$_lang['nuovo'] = "nuovo";
$_lang['nuova'] = "nuova";
$_lang['nuovi'] = "nuovi";
$_lang['nuove'] = "nuove";
$_lang['ultimo'] = "ultimo";
$_lang['ultima'] = "ultima";
$_lang['ultimi'] = "ultimi";
$_lang['ultime'] = "ultime";
$_lang['primo'] = "primo";
$_lang['prima'] = "prima";
$_lang['primi'] = "primi";
$_lang['prime'] = "prime";
$_lang['tutto'] = "tutto";
$_lang['tutti'] = "tutti";
$_lang['tutte'] = "tutte";
$_lang['Sei sicuro?'] = "Sei sicuro?";
$_lang['caricamento'] = "caricamento";
$_lang['processo dati'] = "processo dati";
$_lang['profilo'] = "profilo";
$_lang['logout'] = "logout";
$_lang['dimenticati'] = "dimenticati";
$_lang['completato'] = "completato";
$_lang['anagrafica'] = "anagrafica";
$_lang['accesso'] = "accesso";
$_lang['tipo accesso'] = "tipo accesso";
$_lang['pubblico'] = "pubblico";
$_lang['privato'] = "privato";
$_lang['amministratore'] = "amministratore";
$_lang['amministrazione'] = "amministrazione";
$_lang['nr.'] = "nr.";
$_lang['del'] = 'del';
$_lang['tipo'] = "tipo";
$_lang['moduli disponibili'] = "moduli disponibili";
$_lang['numero'] = "numero";
$_lang['data'] = "data";
$_lang['data scadenza'] = "data scadenza";
$_lang['scadenza'] = "scadenza";
$_lang['scadenze'] = "scadenze";
$_lang['inizio'] = "inizio";
$_lang['fine'] = "fine";
$_lang['tempo'] = "tempo";
$_lang['tempo totale'] = "tempo totale";
$_lang['immagine zoom'] = "immagine zoom";
$_lang['immagine'] = "immagine";
$_lang['immagini'] = "immagini";

$_lang['partita IVA'] = "partita IVA";
$_lang['P. IVA'] = "P. IVA";
$_lang['P.IVA'] = "P.IVA";
$_lang['IVA'] = "IVA";
$_lang['iva'] = "iva";
$_lang['codice fiscale'] = "codice fiscale";
$_lang['C. Fiscale'] = "C. Fiscale";
$_lang['C.F.'] = "C.F.";
$_lang['ragione sociale'] = "ragione sociale";
$_lang['nome'] = "nome";
$_lang['cognome'] = "cognome";
$_lang['via'] = "via";
$_lang['cap'] = "C.A.P.";
$_lang['c.a.p.'] = "C.A.P.";
$_lang['città'] = "città";
$_lang['comune'] = "comune";
$_lang['comuni'] = "comuni";
$_lang['provincia'] = "provincia";
$_lang['province'] = "province";
$_lang['targa'] = "targa";
$_lang['stato'] = "stato";
$_lang['stati'] = "stati";
$_lang['nazione'] = "nazione";
$_lang['nazioni'] = "nazioni";
$_lang['telefono'] = "telefono";
$_lang['fax'] = "fax";
$_lang['cellulare'] = "cellulare";
$_lang['numero di telefono'] = "numero di telefono";
$_lang['numero di cellulare'] = "numero di cellulare";
$_lang['numero di fax'] = "numero di fax";
$_lang['skype'] = "skype";
$_lang['nome utente skype'] = "nome utente skype";
$_lang['genitore'] = "genitore";
$_lang['fiscale'] = "fiscale";
$_lang['banca'] = "banca";
$_lang['fatturazione'] = "fatturazione";
$_lang['testo no IVA'] = "testo no IVA";
$_lang['testo rivalsa'] = "testo rivalsa";
$_lang['intestatario'] = "intestatario";
$_lang['iban'] = "iban";
$_lang['bic swift'] = "bic swift";
$_lang['rivalsa'] = "rivalsa";
$_lang['riepilogo IVA'] = "riepilogo IVA";
$_lang['gestione IVA'] = "gestione IVA";
$_lang['gestione rivalsa'] = "gestione rivalsa";
$_lang['email'] = "email";
$_lang['indirizzo email'] = "indirizzo email";
$_lang['indirizzo email valido'] = "indirizzo email valido";
$_lang['nota'] = "nota";
$_lang['soggetto terzo'] = "soggetto terzo";
$_lang['soggetti terzi'] = "soggetti terzi";
$_lang['titolo'] = "titolo";
$_lang['descrizione'] = "descrizione";
$_lang['contenuto'] = "contenuto";
$_lang['contenuti'] = "contenuti";
$_lang['contenuto HTML'] = "contenuto HTML";
$_lang['contenuti HTML'] = "contenuti HTML";
$_lang['creazione'] = "creazione";
$_lang['accesso scrittura'] = "accesso scrittura";
$_lang['accesso lettura'] = "accesso lettura";
$_lang['scrittura'] = "scrittura";
$_lang['lettura'] = "lettura";
$_lang['template'] = "template";
$_lang['proprietario'] = "proprietario";
$_lang['dati base'] = "dati base";
$_lang['opzioni'] = "opzioni";
$_lang['percentuale di completamento'] = "percentuale di completamento";
$_lang['completato'] = "completamento";
$_lang['completato - abb'] = "compl.";
$_lang['selezionato'] = "selezionato";
$_lang['status'] = "status";
$_lang['costo orario'] = "costo orario";
$_lang['quantità'] = "quantità";
$_lang['q.tà'] = "q.tà";
$_lang['tassa'] = "tassa";
$_lang['imponibile'] = "imponibile";
$_lang['imposte'] = "imposte";
$_lang['prezzo'] = "prezzo";
$_lang['prezzo unità'] = "prezzo unità";
$_lang['prezzo unitario'] = "prezzo unitario";
$_lang['prezzo totale'] = "prezzo totale";
$_lang['totale'] = "totale";
$_lang['totali'] = "totali";
$_lang['totale onorario'] = "totale onorario";
$_lang['importo'] = "importo";
$_lang['importo lordo'] = "importo lordo";
$_lang['pagamento'] = 'pagamento';
$_lang['modalità pagamento'] = "modalità pagamento";
$_lang['bonifico bancario'] = "bonifico bancario";
$_lang['destinatario'] = "destinatario";
$_lang['lista %ITEMS%'] = "lista %ITEMS%";
$_lang['lista dei %ITEMS%'] = "lista dei %ITEMS%";
$_lang['lista degli %ITEMS%'] = "lista degli %ITEMS%";
$_lang['lista delle %ITEMS%'] = "lista delle %ITEMS%";
$_lang['torna al %ITEM%'] = "torna al %ITEM%";
$_lang['torna alla lista %ITEM%'] = "torna alla lista %ITEM%";
$_lang['torna alla pagina %ITEM%'] = "torna alla pagina %ITEM%";
$_lang['torna alla pagina'] = "torna alla pagina";
$_lang['nuovo %ITEM%'] = "nuovo %ITEM%";
$_lang['nuova %ITEM%'] = 'nuova %ITEM%';
$_lang['nuovi %ITEMS%'] = 'nuova %ITEM%';
$_lang['nuove %ITEMS%'] = 'nuove %ITEM%';
$_lang['username'] = "username";
$_lang['nome utente'] = "nome utente";
$_lang['avatar'] = "avatar";
$_lang['utente'] = "utente";
$_lang['utenti'] = "utenti";
$_lang['livello'] = "livello";
$_lang['livelli'] = "livelli";
$_lang['password'] = "password";
$_lang['password di controllo'] = "password di controllo";
$_lang['profilo utente'] = "profilo utente";
$_lang['timecard'] = "timecard";
$_lang['progetti'] = "progetti";
$_lang['progetto'] = "progetto";
$_lang['da fare'] = "da fare";
$_lang['moduli'] = "moduli";
$_lang['moduli disponibili'] = "moduli disponibili";
$_lang['moduli attivi'] = "moduli attivi";
$_lang['modulo'] = "modulo";
$_lang['fattura'] = "fattura";
$_lang['fatture'] = "fatture";
$_lang['preventivo'] = "preventivo";
$_lang['preventivi'] = "preventivi";
$_lang['articolo'] = 'articolo';
$_lang['articoli'] = 'articoli';
$_lang['contatto'] = "contatto";
$_lang['contatti'] = "contatti";
$_lang['categoria'] = 'categoria';
$_lang['categorie'] = 'categorie';

$_lang['prodotto'] = "prodotto";
$_lang['prodotti'] = "prodotti";
$_lang['tag'] = "tag";
$_lang['tags'] = "tags";

$_lang['home'] = 'home';
$_lang['impostazioni'] = 'impostazioni';
$_lang['nota'] = 'nota';
$_lang['note'] = 'note';
$_lang['percentuale'] = 'percentuale';
$_lang['percentuali'] = 'percentuali';
$_lang['tabella'] = 'tabella';

$_lang['nome file'] = 'nome file';
$_lang['nome file originale'] = 'nome file originale';
$_lang['attivazione'] = "attivazione";

$_lang['valore'] = "valore";
$_lang['valore stringa'] = "valore stringa";
$_lang['valore intero'] = "valore intero";
$_lang['valore float'] = "valore float";
$_lang['valore tipo'] = "valore tipo";
$_lang['valori'] = "valori";

// actions
$_lang['attivato'] = "attivato";
$_lang['attivata'] = "attivata";
$_lang['disattivato'] = "disattivato";
$_lang['disattivata'] = "disattivata";
$_lang['cancella'] = "cancella";
$_lang['cancella %ITEM%'] = "cancella %ITEM%";
$_lang['edita'] = "edita";
$_lang['edita %ITEM%'] = "edita %ITEM%";
$_lang['attiva'] = "attiva";
$_lang['attiva %ITEM%'] = "attiva %ITEM%";
$_lang['disattiva'] = "disattiva";
$_lang['disattiva %ITEM%'] = "disattiva %ITEM%";
$_lang['sposta %THEITEM% su'] = "sposta %THEITEM% su";
$_lang['sposta %THEITEM% giu'] = "sposta %THEITEM% giu";
$_lang['%ITEM% spostata su'] = "%ITEM% spostata su";
$_lang['%ITEM% spostata giu'] = "%ITEM% spostata giu";
$_lang['spostata'] = "spostata";
$_lang['spostato'] = "spostato";
$_lang['su'] = "su";
$_lang['giu'] = "giu";
$_lang['cerca'] = "cerca";
$_lang['invia'] = "invia";
$_lang['applica'] = "applica";
$_lang['indietro'] = "indietro";
$_lang['resetta'] = "resetta";
$_lang['loggati'] = "login";
$_lang['login'] = "login";
$_lang['chiudi'] = "chiudi";
$_lang['gestisci'] = "gestisci";
$_lang['esporta in pdf'] = "esporta in pdf";
$_lang['esporta in xls'] = "esporta in xls";
$_lang['aggiungi'] = "aggiungi";
$_lang['aggiungi %ITEM%'] = "aggiungi %ITEM%";
$_lang['modifica'] = "modifica";
$_lang['modifica %ITEM%'] = "modifica %ITEM%";
$_lang['modifica il %ITEM%'] = "modifica il %ITEM%";
$_lang['modifica la %ITEM%'] = "modifica la %ITEM%";
$_lang['modifica i %ITEM%'] = "modifica i %ITEM%";
$_lang['modifica gli %ITEM%'] = "modifica gli %ITEM%";
$_lang['modifica la %ITEM%'] = "modifica la %ITEM%";
$_lang['modifica la tua %ITEM%'] = "modifica la tua %ITEM%";
$_lang['modifica profilo utente'] = "modifica profilo utente";

$_lang['inserisci %ITEM%'] = "inserisci %ITEM%";
$_lang['inserisci un %ITEM%'] = "inserisci un %ITEM%";
$_lang['inserisci uno %ITEM%'] = "inserisci uno %ITEM%";
$_lang['inserisci una %ITEM%'] = "inserisci una %ITEM%";

$_lang['inserisci un nuovo %ITEM%'] = "inserisci un nuovo %ITEM%";
$_lang['inserisci una nuova %ITEM%'] = "inserisci una nuova %ITEM%";

$_lang['seleziona uno %ITEM%'] = "seleziona uno %ITEM%";
$_lang['seleziona una %ITEM%'] = "seleziona una %ITEM%";
$_lang['seleziona un %ITEM%'] = "seleziona un %ITEM%";

$_lang['gestisci %ITEM%'] = "gestisci %ITEM%";

/* MESSAGES */
/* indications */
$_lang['vai alla lista'] = "vai alla lista";
$_lang['mostra da %START% a %END% di %ITEM% elementi'] = 'mostra da %START% a %END% di %ITEM% elementi';
$_lang['Mostra _MENU_ voci per pagina'] = "Mostra _MENU_ voci per pagina";
$_lang['Mostra pagina _PAGE_ di _PAGES_'] = "Mostra pagina _PAGE_ di _PAGES_";
$_lang['fitrati da _MAX_ voci totali'] = "fitrati da _MAX_ voci totali";

$_lang['clicca per recuperare lo username'] = "clicca per recuperare lo username";
$_lang['clicca per recuperare la password'] = "clicca per recuperare la password";

$_lang['pronto ad abbandonare?'] = "pronto ad abbandonare?";
$_lang['seleziona logout se sei pronto a terminare la tua sessione corrente'] = 'seleziona "logout" se sei pronto a terminare la tua sessione corrente';


/* confirmations */
$_lang['modifiche effettuate'] = "modifiche effettuate";

$_lang['%ITEM% inserito'] = "%ITEM% inserito";
$_lang['%ITEM% inserita'] = "%ITEM% inserita";

$_lang['%ITEM% modificato'] = "%ITEM% modificato";
$_lang['%ITEM% modificata'] = "%ITEM% modificata";

$_lang['%ITEM% cancellato'] = "%ITEM% cancellato";
$_lang['%ITEM% cancellata'] = "%ITEM% cancellata";

$_lang['%ITEM% attivato'] = "%ITEM% attivato";
$_lang['%ITEM% attivata'] = "%ITEM% attivata";

$_lang['%ITEM% disattivato'] = "%ITEM% disattivato";
$_lang['%ITEM% disattivata'] = "%ITEM% disattivata";

$_lang['username <strong>%USERNAME%</strong> è libero'] = "l'username <strong>%USERNAME%</strong> è libero";
$_lang['indirizzo <strong>%EMAIL%</strong> è libero'] = "l'indirizzo <strong>%EMAIL%</strong> è libero";

$_lang['Account modificato correttamente! Per rendere effettive le modifiche devi uscire dal sistema e loggarti nuovamente.'] = "Account modificato correttamente! Per rendere effettive le modifiche devi uscire dal sistema e loggarti nuovamente.";
$_lang['Password modificata correttamente! Sarà effettiva al prossimo login.'] = "Password modificata correttamente! Sarà effettiva al prossimo login.";

/* errors */
$_lang['Il file indicato è troppo grande! Il file deve avere la dimensione massima di %MAZSIZE% KByte! Se esisteva è stato mantenuto il file precedente.'] = "Il file indicato è troppo grande! Il file deve avere la dimensione massima di %MAZSIZE% KByte! Se esisteva è stato mantenuto il file precedente.";
$_lang['Le due password non corrispondono! Sarà comunque mantenuta quella precedentemente memorizzata.'] = "Le due password non corrispondono! Sarà comunque mantenuta quella precedentemente memorizzata.";
$_lang['Le due password non corrispondono!'] = "Le due password non corrispondono!";
$_lang['nessuna voce trovata!'] = "nessuna voce trovata!";

$_lang['inserisci username e password'] = "inserisci username e password";

$_lang['Devi inserire una %ITEM%!'] = "Devi inserire una %ITEM%!";
$_lang['Devi inserire un %ITEM%!'] = "Devi inserire un %ITEM%!";

$_lang['Devi selezionare una %ITEM%!'] = "Devi selezionare una %ITEM%!";
$_lang['Devi selezionare un %ITEM%!'] = "Devi selezionare un %ITEM%!";

$_lang['Devi inserire il campo %FIELD%!'] = " Devi inserire il campo %FIELD%!";

$_lang['Inserisci una %ITEM%!'] = "Inserisci una %ITEM%!";
$_lang['Inserisci un %ITEM%!'] = "Inserisci un %ITEM%!";

$_lang['Devi creare o attivare almeno un %ITEM%'] = "Devi creare o attivare almeno un %ITEM%";
$_lang['Devi creare o attivare almeno una %ITEM%'] = "Devi creare o attivare almeno una %ITEM%";

$_lang['Errore accesso db!'] = "Errore accesso db!";
$_lang['Accesso negato!'] = "Accesso negato!";
$_lang['I dati inseriti non corrispondono!'] = "I dati inseriti non corrispondono!";

$_lang['username <strong>%USERNAME%</strong> risulta già presente nel nostro database'] = "l'username <strong>%USERNAME%</strong> risulta già presente nel nostro database";
$_lang['indirizzo <strong>%EMAIL%</strong> risulta già presente nel nostro database'] = "l'indirizzo <strong>%EMAIL%</strong> risulta già presente nel nostro database";

$_lang['Non è possibile diminuire ordinamento!'] = "Non è possibile diminuire ordinamento!";
$_lang['Non è possibile aumentare ordinamento!'] = "Non è possibile aumentare ordinamento!";

$_lang['Errore! Ci sono ancora figli associati!'] = "Errore! Ci sono ancora figli associati!";
$_lang['Errore! Ci sono ancora voci associate!'] = "Errore! Ci sono ancora voci associate!";
$_lang['Errore! Ci sono ancora %ITEM% associati!'] = "Errore! Ci sono ancora %ITEM% associati!";
$_lang['Errore! Ci sono ancora %ITEM% associate!'] = "Errore! Ci sono ancora %ITEM% associate!";

$_lang['Intervallo di tempo si sovrappone ad un altro inserito nella stessa data!'] = "Intervallo di tempo si sovrappone ad un altro inserito nella stessa data!";
$_lang['Intervallo tra le due date errato!'] = "Intervallo tra le due date errato!";
$_lang['La data inserita non è valida!'] = "La data inserita non è valida!";

$_lang['Errore nel modulo %MODULE%!'] = "Errore nel modulo %MODULE%!";
$_lang['Errore!'] = "Errore!";
$_lang['Internal Server Error!'] = "Errore Interno Server";
$_lang['Access Error!'] = "Accesso Negato!";
$_lang['testo errore generico'] = "Il server ha riscontrato qualcosa di inaspettato che non gli ha permesso di completare la richiesta. Ci scusiamo.";
$_lang['testo errore accesso'] = "Non hai i permessi per accedere alla risorsa richiesta.";
$_lang['Error 404!'] = "Errore 404!";
$_lang['Database Error!'] = "Errore database!";
$_lang['Mail Error!'] = "Errore invio mail!";
$_lang['Module Error!'] = "Errore modulo!";
$_lang['Permissions Error!'] = "Errore permessi!";

$_lang['testo errore generico'] = "Il server ha riscontrato qualcosa di inaspettato che non gli ha permesso di completare la richiesta. Ci scusiamo.";
$_lang['testo errore accesso'] = "Non hai i permessi per accedere alla risorsa richiesta.";
$_lang['testo errore 404'] = "Ci dispiace ma la pagina che state cercando non esiste!";
$_lang['testo errore database'] = "Ci sono errori nell'accesso al database oppure alle query per la gestione dei dati!";
$_lang['testo errore mail'] = "Non è stato possibile inviare emails per un problema di server!";
$_lang['testo errore permessi'] = "Non hai il permesso per fare l'azione richiesta!";

$_lang['titolo sezione richiesta username'] = "Richiesta nome utente";

$_lang['testo intro sezione richiesta username'] = "Dopo aver compilato correttamente i campi il sistema cercherà il nome utente associato all'indirizzo email indicato e vi sarà inviato.<br>Se dopo la fine della procedura non riceverete l'email controllate che essa non sia nel filtro antispan (se presente) oppure contattare l'amministratore.";
$_lang['Indirizzo email inserito non esiste! Vi invitiamo a ripetere la procedura o contattare amministratore del sistema.'] = "L'indirizzo email inserito non esiste! Vi invitiamo a ripetere la procedura o contattare l'amministratore del sistema.";
$_lang['Errore invio della email! Vi invitiamo a ripetere la procedura o contattare amministratore.'] = "Errore nell'invio della email! Vi invitiamo a ripetere la procedura o contattare l'amministratore del sistema.";
$_lang['Email inviata correttamente! Nel testo troverete il username!'] = "Email inviata correttamente! Nel testo troverete il nome utente!";
$_lang['titolo email sezione richiesta username'] = "Invio nome utente dimenticato dal sito %SITENAME%";
$_lang['testo email sezione richiesta username'] = "<p>Come da richiesta le inviamo il nome utente iscritto al sito <b>%SITENAME%</b> che è associato all'indirizzo email: <b>%EMAIL%</b></p><p>Nome utente: <b>%USERNAME%</b></p>";

$_lang['titolo sezione richiesta password'] = "Richiesta Password";
$_lang['testo intro sezione richiesta password'] = "Dopo aver compilato correttamente i campi il sistema genererà una password casuale che vi sarà inviata nella email indicata nel profilo.<br>Se dopo la fine delle procedura non riceverete l'email controllate che essa non sia nel filtro antispan (se presente) oppure contattare l'amministratore.";
$_lang['Il nome utente inserito non esiste! Vi invitiamo a ripetere la procedura o contattare amministratore del sistema.'] = "Il nome utente inserito non esiste! Vi invitiamo a ripetere la procedura o contattare l'amministratore del sistema.";
$_lang['nopassword core - errore invio email'] = "Errore nell'invio della email! Vi invitiamo a ripetere la procedura o contattare l'amministratore del sistema.";
$_lang['La nuova password vi è stata inviata con email indirizzo associato ed è stata memorizzata nel sistema!'] = "La nuova password vi è stata inviata con un'email all'indirizzo associato ed è stata memorizzata nel sistema!";
$_lang['Errore database! La nuova password NON è stata memorizzata nel sistema! Vi invitiamo a ripetere la procedura o contattare amministratore'] = "Errore database! La nuova password NON è stata memorizzata nel sistema! Vi invitiamo a ripetere la procedura o contattare l'amministratore del sistema.";
$_lang['titolo email sezione richiesta password'] = "Invio password dimenticata dal sito %SITENAME%";
$_lang['testo email sezione richiesta password'] = "<p>Come da richiesta le inviamo la nuova password associata all'utente <b>%USERNAME%</b> iscritto al sito <b>%SITENAME%</b></p><p>Password: <b>%PASSWORD%</b></p>";
?>
