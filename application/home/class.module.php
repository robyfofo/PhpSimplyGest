...
* app/site-home/lass.module.php v.1.0.0. 13/02/2018
*/
...
class Module {
	...
	public function getItemBlockUrl($arr,$lastLogin) {
		/* preleva i dati del lla prima voce */
		$whereclause = (isset($arr['whereclause']) && $arr['whereclause'] != '' ? $arr['whereclause'] : 'created > ?');		
		Sql::initQuery($arr['table'],array('*'),array($lastLogin),$whereclause,'','',false);		
		$itemData = Sql::getRecord();		
		$str = $arr['url item']['string'];
		$opz = $arr['url item']['opz'];
		$str = preg_replace('/%URLSITE%/',URL_SITE,$str);	
